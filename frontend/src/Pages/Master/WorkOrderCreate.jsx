...
export default function WorkOrderCreate() {
  ...
  const handleDeliveryDateChange = (e) => {
    ...
    if (selectedDate < today) {
      toast.warning("Delivery date cannot be in the past");
      return;
    }

    setForm({ ...form, deliveryDate: selectedDate });
  };

  // this is for po date validation
  const handlePoDateChange = (e) => {
    ...
    setForm({ ...form, poDate: selectedDate });
  };
  ...
  const handleSave = async () => {
    if (
      ...
      return toast.warning(
        "Please fill all mandatory fields (Target Location, Division, Order Type, and Delivery Date).",
      );
    }

    const invalidProducts = form.products.some(
      (p) =>
        !p.categoryId || !p.productId || !p.quantity || Number(p.quantity) <= 0,
    );
    if (invalidProducts) {
      return toast.warning(
        "Please ensure all items have a category, product, and a valid quantity greater than 0.",
      );
    }

    if (!window.confirm("Are you sure you want to save this work order?"))
      return;
    ...
    try {
      ...
      toast.error(
        err.response?.data?.message ||
          "Failed to create Work Order. Please check your inputs.",
      );
    } finally {
      setLoading(false);
    }
      ...
      <div className="card bg-dark-glass text-white border-0 shadow-lg mb-4">
        <div className="card-header bg-transparent border-bottom border-secondary p-3">
          <h4 className="mb-0 fw-bold text-info">
            Create Work Order ({auth?.divisionName})
          </h4>
        </div>
        <div className="card-body p-4">
          <div className="row g-3">
            <div className="col-md-3">
              ...
              <input
                className="form-control bg-dark text-info border-secondary fw-bold"
            ...
            <div className="col-md-3">
              ...
              <select
                ...
                onChange={(e) =>
                  setForm({ ...form, orderTypeId: e.target.value })
                }
                disabled={loading}
              >
            ...
            <div className="col-md-3">
              ...
              <select
                ...
                onChange={(e) =>
                  setForm({ ...form, toDivisionId: e.target.value })
                }
                disabled={loading}
              >
                ...
                {divisions
                  .filter(
                    (d) => Number(d.locationId) === Number(form.toLocationId),
                  )
                  .map((d) => (
            ...
            <div className="col-md-6">
              <label className="form-label small fw-bold">
                Marketing Person (Authorized Name)
              </label>
              <input
                className="form-control bg-dark text-white border-secondary"
                  ...
                  <tr key={i}>
                    ...
                    <td className="text-center">
                      <button
                        ...
                        disabled={form.products.length === 1 || loading}
                      >
